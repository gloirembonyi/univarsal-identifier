...
export default function UniversalIdentifier() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#1A237E] to-[#000051] text-white font-['Roboto']">
      <UniversalIdentifierStyles />
          ...
          <div className="bg-[#0D47A1]/80 rounded-lg shadow-lg p-6 mb-8 backdrop-blur-sm">
            ...
            {showResults && result && !isAnalysisHidden && (
              <div className={`transition-all duration-300 ease-in-out ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <div className="bg-[#0D47A1] p-6 rounded-lg shadow-lg backdrop-blur-sm transform transition-all duration-300 overflow-y-auto max-h-[600px]">
                  <h3 className="text-2xl font-semibold text-[#2196F3] mb-4">Analysis Results</h3>
                  <div className="space-y-4">
                    {result.split('\n\n').map((section, index) => {
                      const [title, ...content] = section.split('\n')
                      return (
                        <div key={index} className="mb-4">
                          <h4 className="text-xl font-semibold text-[#64B5F6] mb-2">{title}</h4>
                          <ul className="list-disc list-inside space-y-1">
                            {content.map((item, i) => (
                              <li key={i} className="text-white font-light">{item.trim()}</li>
                            ))}
                          </ul>
                        </div>
                      )
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <button
                  ...
                  className="bg-[#2196F3] text-white px-3 py-1 rounded hover:bg-[#1E88E5] transition font-light text-sm flex items-center"
                >
                  <FaExpand className="mr-1" /> Expand
                </button>
              </div>
              
            {chatMessages.length > 0 ? (
              <div className="h-64 overflow-y-auto mb-4 bg-[#1A237E]/50 rounded-lg p-4">
                {chatMessages.map((msg, index) => (
                  <div
                    key={index}
                    className={`mb-2 ${
              ...
              <div className="h-16 flex items-center justify-center text-[#64B5F6]">
